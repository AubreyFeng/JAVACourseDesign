...
public class BookDAO {
	...
	public static boolean updateReader(Book book) {
		return JDBCUtil.exeUpdate(
				"update book set ISBN = ?,bName = ?,Category = ?,author = ?,press = ?,publicationDate = ?,recordDate = ? where ISBN = ?",
				book.getISBN(), book.getName(), book.getCategory(), book.getAuthor(), book.getPress(),
				book.getPublicationDate(), book.getRecordDate(), book.getISBN());
	}
